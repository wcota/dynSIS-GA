! ## File: dynamics.f90
! ## - main program: running the SIS dynamics, based on GA (Gillespie Algorithm).
...
! This code is available at <https://github.com/wcota/dynSIS-GA>
! 
! The optimized version are also available:
! Fortran implementation at <https://github.com/wcota/dynSIS>
...
implicit none
    ...
    ! SIS-GA - Dynamics Variables
    real*8                        :: dyn_m                  ! m = M/R. 1 - m = w = W/R
    real*8                        :: dyn_L                  ! Rate L = lambda*N_IS
    real*8                        :: dyn_R                  ! Total rate
    integer, allocatable          :: dyn_VI(:), dyn_VIS(:), dyn_sig(:)  ! Lists V^I, V^{IS} and sigma
    integer                       :: dyn_NI, dyn_NIS        ! # of infected vertex N_I and # of edges IS N_IS
...
contains
 ...
 & each sample): ")
        ...
        allocate(dyn_VI(net_N),dyn_VIS(net_skk),dyn_sig(net_N))
    end subroutine
    ...
    subroutine random_initial_condition()
        ...
        !dyn_VIS = 0 ! list V^IS (not needed)
        dyn_NI = 0 ! N_I
        dyn_NIS = 0  ! N_IS
    ...
    subroutine dyn_run()
        ...
        dyn_time_loop : do while (dyn_t <= dynp_tmax)
            ...
            dyn_NIS = 0
            ! Loop over all the V^I list
            do a = 1, dyn_NI
                ver = dyn_VI(a)
                ! Check the susceptible neighbors
                do b = net_ini(ver) , net_ini(ver) + net_k(ver) - 1
                    c = net_con(b)
                    if ( dyn_sig(c) == 0 ) then
                        dyn_NIS = dyn_NIS + 1
                        dyn_VIS(dyn_NIS) = c
                ...
                enddo
            enddo
            dyn_L = dynp_lb*dyn_NIS
            
            ! Calculate the total rate
            dyn_R = (dyn_NI + dyn_L) ! mu = 1 in this case. dyn_M = dyn_NI
            ...
            ! If not, try to infect: w = 1 - m
                ...
                ! Select one vertex from list V^{IS} and infect it
                pos_nei = random_int(1,dyn_NIS)
                ver = dyn_VIS(pos_nei)
    ...
    subroutine run_prog()
        ...
        call print_info('############ processes on large and heterogeneous networks: SIS-GA  ############')
        ...
        do dynp_i=1,dynp_sam
            ...
            write(und_output,'(a)')         "## ***** Algorithm used: Gillespie Algorithm for SIS &
                                            & (SIS-GA, Fortran) [not optimized] *****"
        ...
        call print_info('')
        call print_info('*****Algorithm used: Gillespie Algorithm for SIS (SIS-GA, Fortran) [not optimized]*****')
